...
impl BTreeCursor {
    ...
    pub fn new_table(
        ...
        Self::new(mv_cursor, btree, root_page, Vec::new())
    }
    ...
    pub fn new_index(
        ...
        let mut cursor = Self::new(mv_cursor, btree, root_page, collations);
    ...
    fn is_empty_table(&self) -> Result<CursorResult<bool>> {
        ...
        let page = self.btree.pager.read_page(self.root_page)?;
    ...
    fn get_prev_record(&mut self) -> Result<CursorResult<bool>> {
        loop {
            ...
            return_if_locked_maybe_load!(self.btree.pager, page);
            let page = page.get();
    ...
    fn process_overflow_read(
        ...
        let usable_space = self.btree.usable_space();
    ...
    pub fn read_write_payload_with_offset(
        ...
        return_if_locked_maybe_load!(self.btree.pager, page_btree);
    ...
    fn write_payload_to_page(
        ...
        page.get().set_dirty();
        self.btree.pager.add_dirty(page.get().get().id);
    ...
    fn get_next_record(&mut self) -> Result<CursorResult<bool>> {
        ...
        loop {
            ...
            return_if_locked_maybe_load!(self.btree.pager, mem_page_rc);
            ...
            match write_state {
                WriteState::Start => {
                    ...
                    let (cell_idx, page_type) = {
                        ...
                        page.set_dirty();
                        self.btree.pager.add_dirty(page.get().id);
    ...
    fn balance_non_root(&mut self) -> Result<CursorResult<()>> {
        ...
        let (next_write_state, result) = match state {
            ...
            WriteState::BalanceNonRoot => {
                ...
                return_if_locked_maybe_load!(self.btree.pager, parent_page);
                ...
                self.btree.pager.add_dirty(parent_page.get().id);
                ...
                for i in (0..=current_sibling).rev() {
                    let page = self.read_page(pgno as usize)?;
                    {
                        ...
                        self.btree.pager.add_dirty(sibling_page.get().id);
                    }
                    #[cfg(debug_assertions)]
                    {
                        return_if_locked!(page.get());
                ...
                #[cfg(debug_assertions)]
                {
                    ...
                    for page in pages_to_balance.iter().take(sibling_count) {
                        return_if_locked_maybe_load!(self.btree.pager, page.as_ref().unwrap());
            ...
            WriteState::BalanceNonRootWaitLoadPages => {
                ...
                */
                ...
                /* do not take last page */
                {
                    ...
                    assert!(
                        left_pointer <= header_accessor::get_database_size(&self.btree.pager)?,
    ...
    fn balance_root(&mut self) {
        ...
        self.btree.pager.add_dirty(root.get().id);
        self.btree.pager.add_dirty(child_btree.get().get().id);
        ...
        loop {
            ...
            match delete_state {
                ...
                DeleteState::InteriorNodeReplacement {
                    ...
                    self.btree.pager.add_dirty(parent_page.get().get().id);
                    leaf_page.get().set_dirty();
                    self.btree.pager.add_dirty(leaf_page.get().get().id);
        ...
        loop {
            ...
            match state {
                ...
                OverflowState::ProcessPage { next_page } => {
                    if next_page < 2
                        || next_page as usize
                            > header_accessor::get_database_size(&self.btree.pager)? as usize
                    {
                    ...
                    self.btree.free_page(Some(page), next_page as usize)?;
        ...
        loop {
            ...
            match destroy_state {
                ...
                DestroyState::FreePage => {
                    ...
                    self.btree.free_page(Some(page.get()), page_id)?;
    ...
    pub fn read_page(&self, page_idx: usize) -> Result<BTreePage> {
        self.btree.do_read_page(page_idx)
    }
...
pub fn integrity_check(
    ...
    let page = btree.do_read_page(page_idx)?;
    return_if_locked_maybe_load!(&btree.pager, page);
    state.page_stack.pop();

    let page = page.get();
    let contents = page.get_contents();
    let usable_space = btree.usable_space() as u16;
...
fn fill_cell_payload(
    ...
    loop {
        ...
        // FIXME: handle page cache is full
        let overflow_page = btree.allocate_overflow_page();
...
mod tests {
    ...
    fn validate_btree(btree: Rc<BTree>, page_idx: usize) -> (usize, bool) {
        let cursor = BTreeCursor::new_table(None, btree.clone(), page_idx);
        ...
        let mut valid = true;
        let mut depth = None;
        debug_validate_cells!(contents, btree.usable_space() as u16);
            ...
            let current_depth = match cell {
                ...
                BTreeCell::TableInteriorCell(TableInteriorCell {
                    ...
                    let (child_depth, child_valid) =
                        validate_btree(btree.clone(), _left_child_page as usize);
                    valid &= child_valid;
                    child_depth
                }
        ...
        if let Some(right) = contents.rightmost_pointer() {
            let (right_depth, right_valid) = validate_btree(btree.clone(), right as usize);
            valid &= right_valid;
        ...
        let first_page_type = child_pages.first().map(|p| {
            if !p.get().is_loaded() {
                let new_page = btree.pager.read_page(p.get().get().id).unwrap();
        ...
        if let Some(child_type) = first_page_type {
            for page in child_pages.iter().skip(1) {
                if !page.get().is_loaded() {
                    let new_page = btree.pager.read_page(page.get().get().id).unwrap();
    ...
    fn format_btree(btree: Rc<BTree>, page_idx: usize, depth: usize) -> String {
        let cursor = BTreeCursor::new_table(None, btree.clone(), page_idx);
        let page = cursor.read_page(page_idx).unwrap();
        while page.get().is_locked() {
            btree.pager.io.run_once().unwrap();
        ...
        if let Some(rightmost) = contents.rightmost_pointer() {
            child.push(format_btree(btree.clone(), rightmost as usize, depth + 2));
        }
        let current = format!(
            "{}-page:{}, ptr(right):{}\n{}+cells:{}",
            " ".repeat(depth),
    ...
    pub fn btree_insert_fuzz_ex() {
        for sequence in [
            ...
            let (btree, root_page) = empty_btree();
            let mut cursor = BTreeCursor::new_table(None, btree.clone(), root_page);
            for (key, size) in sequence.iter() {
                ...
                tracing::info!(
                    "=========== btree ===========\n{}\n\n",
                    format_btree(btree.clone(), root_page, 0)
                );
            }
    ...
    fn btree_insert_fuzz_run(
        ...
        for _ in 0..attempts {
            ...
            for insert_id in 0..inserts {
                ...
                run_until_done(|| btree.pager.begin_read_tx(), &btree.pager).unwrap();
                run_until_done(|| btree.pager.begin_write_tx(), &btree.pager).unwrap();
                ...
                .unwrap();
                loop {
                    match btree.pager.end_tx(false).unwrap() {
                ...
                if do_validate {
                    ...
                    for key in keys.iter() {
                        ...
                        run_until_done(|| cursor.next(), btree.pager.deref()).unwrap();
                        let cursor_rowid = run_until_done(|| cursor.rowid(), btree.pager.deref())
                            .unwrap()
                            .unwrap();
                        if *key != cursor_rowid {
                            valid = false;
                ...
                if do_validate
                    && (!valid || matches!(validate_btree(btree.clone(), root_page), (_, false)))
                {
                    let btree_after = format_btree(btree.clone(), root_page, 0);
                    println!("btree before:\n{}", btree_before);
                    println!("btree after:\n{}", btree_after);
                    panic!("invalid btree");
                }
                btree.pager.end_read_tx().unwrap();
            }
            run_until_done(|| btree.pager.begin_read_tx(), &btree.pager).unwrap();
            tracing::info!(
                "=========== btree ===========\n{}\n\n",
            ...
            if matches!(validate_btree(btree.clone(), root_page), (_, false)) {
                panic!("invalid btree");
            }
            cursor.move_to_root();
            for key in keys.iter() {
    ...
    fn btree_index_insert_fuzz_run(attempts: usize, inserts: usize) {
        ...
        tracing::info!("super seed: {}", seed);
        for _ in 0..attempts {
            let (btree, _) = empty_btree();
            let index_root_page_result = btree.create(&CreateBTreeFlags::new_index()).unwrap();
            ...
            let mut cursor = BTreeCursor::new_table(None, btree.clone(), index_root_page);
            let mut keys = SortedVec::new();
            tracing::info!("seed: {}", seed);
            for i in 0..inserts {
                btree.pager.begin_read_tx().unwrap();
                btree.pager.begin_write_tx().unwrap();
                let key = {
                    let result;
                    loop {
                        let cols = (0..10)
            ...
            cursor.move_to_root();
            let mut count = 0;
            while run_until_done(|| cursor.next(), btree.pager.deref()).unwrap() {
                count += 1;
            }
            assert_eq!(
                count,
            ...
            for (i, key) in keys.iter().enumerate() {
                ...
                run_until_done(|| cursor.next(), btree.pager.deref()).unwrap();
                let record = run_until_done(|| cursor.record(), &btree.pager).unwrap();
                let record = record.as_ref().unwrap();
    ...
    #[allow(clippy::arc_with_non_send_sync)]
    fn setup_test_env(database_size: u32) -> Rc<BTree> {
        let page_size = 512;
        ...
        let _ = run_until_done(|| btree.pager.allocate_page1(), &btree.pager);
        for _ in 0..(database_size - 1) {
            btree.allocate_page().unwrap();
        }

        header_accessor::set_page_size(&btree.pager, page_size as u16).unwrap();
    ...
    pub fn test_clear_overflow_pages() -> Result<()> {
        let btree = setup_test_env(5);
        let mut cursor = BTreeCursor::new_table(None, btree.clone(), 1);
        ...
        while current_page <= 4 {
            ...
            let buf = Arc::new(RefCell::new(Buffer::allocate(
                header_accessor::get_page_size(&btree.pager)? as usize,
                drop_fn,
            )));
            ...
            btree.pager.io.run_once()?;

            let page = cursor.read_page(current_page as usize)?;
            while page.get().is_locked() {
                cursor.btree.pager.io.run_once()?;
            }
        ...
        let initial_freelist_pages = header_accessor::get_freelist_pages(&btree.pager)?;
        ...
        match clear_result {
            CursorResult::Ok(_) => {
                ...
                assert_eq!(
                    header_accessor::get_freelist_pages(&btree.pager)?,
                    initial_freelist_pages + 3,
                    "Expected 3 pages to be added to freelist"
                );

                // If this is first trunk page
                let trunk_page_id = header_accessor::get_freelist_trunk_page(&btree.pager)?;
                if trunk_page_id > 0 {
    ...
    pub fn test_clear_overflow_pages_no_overflow() -> Result<()> {
        ...
        match clear_result {
            CursorResult::Ok(_) => {
                ...
                assert_eq!(
                    header_accessor::get_freelist_trunk_page(&btree.pager)?,
                    0,
    ...
    fn test_btree_destroy() -> Result<()> {
        let initial_size = 1;
        let btree = setup_test_env(initial_size);
        let mut cursor = BTreeCursor::new_table(None, btree.clone(), 2);
        ...
        assert_eq!(
            header_accessor::get_database_size(&btree.pager)?,
            4, // We should have pages 1-4
            "Database should have 4 pages total"
        );

        // Track freelist state before destruction
        let initial_free_pages = header_accessor::get_freelist_pages(&btree.pager)?;
        assert_eq!(initial_free_pages, 0, "should start with no free pages");

        run_until_done(|| cursor.btree_destroy(), btree.pager.deref())?;

        let pages_freed = header_accessor::get_freelist_pages(&btree.pager)? - initial_free_pages;
...
"INSERT INTO super_becky VALUES (X'7368696E696E675F736F6C69646169726573', X'666561726C6573735F63617264616E', -170727879.20838165, 2744601113.384678, 5676912434.941502, 6757573601.657997), (X'636F75726167656F75735F706C616E636865', X'696E646570656E64656E745F636172736F6E', -6271723086.761938, -180566679.7470188, -1285774632.134449, 1359665735.7842407), (X'677265676172696F75735F7374616D61746F76', X'7374756E6E696E675F77696C64726F6F7473', -6210238866.953484, 2492683045.8287067, -9688894361.68205, 5420275482.048567), (X'696E646570656E64656E745F6F7267616E697A6572', X'676C6974746572696E675F736F72656C', 9291163783.3073, -6843003475.769236, -1320245894.772686, -5023483808.044955), (X'676C6F77696E675F6E65736963', X'676C65616D696E675F746F726D6579', 829526382.8027191, 9365690945.1316, 4761505764.826195, -4149154965.0024815), (X'616C6C7572696E675F646F637472696E65', X'6E6963655F636C6561766572', 3896644979.981762, -288600448.8016701, 9462856570.130062, -909633752.5993862);",
    ...
    pub fn btree_insert_sequential() {
        ...
        for key in keys.iter() {
            ...
            let exists = run_until_done(|| cursor.exists(&key), btree.pager.deref()).unwrap();
    ...
    pub fn test_delete_balancing() {
        ...
        if let (_, false) = validate_btree(btree.clone(), root_page) {
        ...
        for i in 500..=3500 {
            ...
            if found {
                run_until_done(|| cursor.delete(), btree.pager.deref()).unwrap();
            }
        }
    ...
    pub fn test_overflow_cells() {
        ...
        cursor.move_to_root();
        for i in 0..iterations {
            let has_next = run_until_done(|| cursor.next(), btree.pager.deref()).unwrap();
            if !has_next {
                panic!("expected Some(rowid) but got {:?}", cursor.has_record.get());
            };
            let rowid = run_until_done(|| cursor.rowid(), btree.pager.deref())
                .unwrap()
                .unwrap();
    ...
    pub fn test_read_write_payload_with_offset() {
        let (btree, root_page) = empty_btree();
        let mut cursor = BTreeCursor::new(None, btree.clone(), root_page, vec![]);
    ...
    fn test_free_array() {
        ...
        for _ in 0..ITERATIONS {
            ...
            let page = btree.allocate_page().unwrap();
            let page = Arc::new(BTreePageInner::new(page));
            page.init(page_type, 0, btree.usable_space() as u16);
            let page = page.get();
            let mut size = (rng.next_u64() % 100) as u16;
            let mut i = 0;
            // add a bunch of cells
            while compute_free_space(page.get_contents(), btree.usable_space() as u16) >= size + 10
            {
                insert_cell(i, size, page.get_contents(), btree.clone(), page_type);
                i += 1;
            ...
            #[cfg(debug_assertions)]
            debug_validate_cells_core(contents, btree.usable_space() as u16);
    ...
    fn insert_cell(
        ...
        insert_into_cell(contents, &payload, i as usize, btree.usable_space() as u16).unwrap();
    }
}
